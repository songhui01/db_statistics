...
\begin{comment}
This paragraph, I will change to expalin what statistics is. according to https://learn.microsoft.com/en-us/sql/relational-databases/statistics/statistics?view=sql-server-ver16. and https://www.dbta.com/Columns/DBA-Corner/Database-Statistics-and-Optimization-54605.aspx
The class diagram will be explained related to this one, and why we design this way. There could be other statistics but we do not consider them now. such as Partition-Level Statistics:

Partition Cardinality: The number of distinct values in a partitioned column.
Partition Size: The size of each partition in a partitioned table.
Partition Row Count: The number of rows in each partition.
System-Level Statistics:

Buffer Cache Hit Ratio: The percentage of database requests that are served from memory rather than disk, indicating the efficiency of memory usage.
CPU Usage: The amount of CPU resources consumed by the DBMS, which can affect query performance.
Disk I/O: The amount of read/write operations on the disk, indicating how much data is being transferred between the disk and memory.


And the sarcity is related to ontology of DBS, not to DBS.
\end{comment}
